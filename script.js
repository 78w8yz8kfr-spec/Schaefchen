...
async function createPdf(){
  ...
  try{
    ...
    ensure(38);const signatureGap=8,signatureWidth=(pageW-margin*2-signatureGap)/2,customerX=margin+signatureWidth+signatureGap;doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text('Unterschrift Prüfer',margin,y);doc.text('Unterschrift Auftraggeber / Betreiber',customerX,y);y+=3;doc.setDrawColor(120);doc.rect(margin,y,signatureWidth,28);doc.rect(customerX,y,signatureWidth,28);if(d.sig&&d.sig.startsWith('data:image')){try{doc.addImage(d.sig,'PNG',margin+2,y+2,signatureWidth-4,24,undefined,'FAST')}catch(_){/* Unterschrift bleibt als Feld */}}y+=34;
