...
const stepInfo={
  1:{title:'Auftrag und Prüfdaten',description:'Grunddaten zur Baustelle, Prüfart und zum verwendeten Prüfgerät.'},
...
function uvHtml(uvId,d={}){return `<div class="uv" data-uv="${uvId}"><div class="titleRow sectionHead"><button class="collapseBtn" onclick="toggleSection(this)">▾</button><div class="structureHeading"><small class="uv-index">Verteilung</small><h3 class="uv-title">${esc(d.name||'UV')}</h3></div><div class="structureControls"><button type="button" class="secondary small" onclick="moveUv(this,-1)" title="Nach oben">↑</button><button type="button" class="secondary small" onclick="moveUv(this,1)" title="Nach unten">↓</button><button class="danger small" onclick="removeStructure(this,'.uv','Verteilung')">Löschen</button></div></div><div class="uvBody"><div class="grid"><label>Bezeichnung der Verteilung<input class="uv-name" value="${esc(d.name||'UV')}" placeholder="z. B. UV EG"></label><label>Zuleitung kommt aus<input class="uv-source" value="${esc(d.source||'Hausanschlusskasten')}" placeholder="z. B. HAK, HV, UV EG"></label><label>Kabeltyp Zuleitung${cableSelect('uv-feed-cable', d.feedCable||'')}</label><label>Adernzahl Zuleitung${coreSelect('uv-feed-cores', d.feedCores||'')}</label><label>Querschnitt Zuleitung<span class="unitField"><input class="uv-feed-cross" value="${esc(d.feedCross||'')}" inputmode="decimal" placeholder="z. B. 10"><span>mm²</span></span></label><label>Vorsicherung / Absicherung<input class="uv-feed-fuse" value="${esc(d.feedFuse||d.feed||'')}" placeholder="z. B. 35 A"></label><label>Ort<input class="uv-place" value="${esc(d.place||'')}"></label></div><div class="structureAddMenu"><span>Was möchtest du hinzufügen?</span><div><button onclick="addRcd('${uvId}')">+ FI/RCD-Gruppe</button><button class="secondary" onclick="addCircuit(this.closest('.uv').querySelector('.directCircuits'))">+ Stromkreis ohne FI</button><button class="secondary" onclick="addCircuit(this.closest('.uv').querySelector('.directCircuits'),{device:'fils',name:'FI/LS Stromkreis'})">+ FI/LS-Stromkreis</button></div></div><div class="subhead">FI/RCD-Gruppen</div><div class="rcds"></div><div class="subhead directHead">Direkte Stromkreise und FI/LS</div><div class="directCircuits circuits"></div></div></div>`}
...
function rcdHtml(rcdId,d={}){return `<div class="rcd" data-rcd="${rcdId}"><div class="titleRow sectionHead"><button class="collapseBtn" onclick="toggleSection(this)">▾</button><div class="structureHeading"><small class="rcd-index">FI/RCD</small><h4 class="rcd-title">${esc(d.name||'FI')}</h4></div><div class="structureControls"><button type="button" class="secondary small" onclick="moveRcd(this,-1)" title="Nach oben">↑</button><button type="button" class="secondary small" onclick="moveRcd(this,1)" title="Nach unten">↓</button><button class="danger small" onclick="removeStructure(this,'.rcd','FI/RCD')">Löschen</button></div></div><div class="rcdBody"><div class="grid"><label>Bezeichnung<input class="rcd-name" value="${esc(d.name||'FI')}" placeholder="z. B. FI Bad/Küche"></label><label>Typ<select class="rcd-type"><option ${d.type==='A'?'selected':''}>A</option><option ${d.type==='F'?'selected':''}>F</option><option ${d.type==='B'?'selected':''}>B</option><option ${d.type==='AC'?'selected':''}>AC</option></select></label><label>Charakteristik<select class="rcd-char"><option ${d.char==='unverzögert'?'selected':''}>unverzögert</option><option ${d.char==='G / kurzzeitverzögert'?'selected':''}>G / kurzzeitverzögert</option><option ${d.char==='S / selektiv'?'selected':''}>S / selektiv</option></select></label><label>Bemessungsstrom In<select class="rcd-in"><option>25 A</option><option ${d.inn==='40 A'?'selected':''}>40 A</option><option ${d.inn==='63 A'?'selected':''}>63 A</option><option ${d.inn==='80 A'?'selected':''}>80 A</option></select></label><label>Bemessungsdifferenzstrom IΔn<select class="rcd-idn"><option ${d.idn==='10'?'selected':''} value="10">10 mA</option><option ${!d.idn||d.idn==='30'?'selected':''} value="30">30 mA</option><option ${d.idn==='100'?'selected':''} value="100">100 mA</option><option ${d.idn==='300'?'selected':''} value="300">300 mA</option></select></label><label class="checkLabel">Prüftaste<input class="rcd-test" type="checkbox" ${d.test?'checked':''}></label></div><div class="status neutral rcd-status">FI-Bewertung fehlt</div><div class="inlineAdd"><button onclick="addCircuit(this.closest('.rcd').querySelector('.circuits'))">+ Stromkreis nach diesem FI</button></div><div class="circuits"></div></div></div>`}
function addCircuit(container,data={}){if(typeof container==='string')container=document.querySelector(container);data=data||{};data.insideRcd=!!container.closest('.rcd');if(data.insideRcd&&data.device==='fils')data.device='ls';container.insertAdjacentHTML('beforeend',circuitHtml(data));const el=container.lastElementChild;updateCircuitVisibility(el);updateStructureLabels();evalAll();updateStepProgress();saveData();}
...
function updateStructureLabels(){
  document.querySelectorAll('.uv').forEach((uv,uvIndex)=>{
    ...
    uv.querySelectorAll(':scope .uvBody > .rcds > .rcd').forEach((rcd,rcdIndex)=>{
      const rcdName=val(rcd,'.rcd-name')||`FI/RCD ${rcdIndex+1}`;const ri=rcd.querySelector('.rcd-index'),rt=rcd.querySelector('.rcd-title');if(ri)ri.textContent=`FI/RCD ${uvNumber}.${rcdIndex+1}`;if(rt)rt.textContent=rcdName;
...
function updateStepProgress(){
  ...
  if(currentStep===1){const selectors=['#kunde','#objekt','#adresse','#datum','#pruefer','#gHersteller','#gTyp','#gSerie'];total=selectors.length+1;completed=selectors.filter(s=>document.querySelector(s)?.value.trim()).length+(document.querySelector('[name=pruefart]:checked')?1:0)}
