...
function preparePrint(){
  ...
  (d.uvs||[]).forEach((u,ui)=>{
    ...
    if((u.direct||[]).length){
      rows += `<tr><td colspan="21"><b>Direkte Stromkreise: LS ohne FI davor oder FI/LS integriert</b></td></tr>`+measHeader();
  ...
  let printCheckIndex=0;const checksHtml=checkGroups.map(group=>`<tr><th colspan="3">${esc(group.title)}</th></tr>`+group.items.map(t=>{const i=printCheckIndex++;return `<tr><td>${esc(t)}</td><td>${d.checks&&d.checks[i]==='io'?'X':''}</td><td>${d.checks&&d.checks[i]==='nio'?'X':''}</td></tr>`}).join('')).join('');
