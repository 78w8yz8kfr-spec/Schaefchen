...
function addUv(data={}){const uvId=data.id||id();document.getElementById('uvs').insertAdjacentHTML('beforeend',uvHtml(uvId,data));const uv=document.querySelector(`[data-uv="${uvId}"]`);(data.rcds||[]).forEach(r=>addRcd(uvId,r));(data.direct||[]).forEach(c=>addCircuit(uv.querySelector('.directCircuits'),c));if(!(data.rcds||[]).length&&!(data.direct||[]).length)addRcd(uvId,{name:'FI 1'});evalAll();}
function uvHtml(uvId,d={}){return `<div class="uv" data-uv="${uvId}"><div class="titleRow sectionHead"><button class="collapseBtn" onclick="toggleSection(this)">▾</button><h3>Unterverteilung</h3><button class="danger small" onclick="this.closest('.uv').remove();evalAll();saveData()">UV löschen</button></div><div class="uvBody"><div class="grid"><label>UV-Bezeichnung<input class="uv-name" value="${esc(d.name||'UV')}" placeholder="z.B. UV EG"></label><label>Zuleitung kommt aus<input class="uv-source" value="${esc(d.source||'Hausanschlusskasten')}" placeholder="z.B. HAK, HV, UV EG"></label><label>Kabeltyp Zuleitung${cableSelect('uv-feed-cable', d.feedCable||'')}</label><label>Adernzahl Zuleitung${coreSelect('uv-feed-cores', d.feedCores||'')}</label><label>Querschnitt Zuleitung<span class="unitField"><input class="uv-feed-cross" value="${esc(d.feedCross||'')}" inputmode="decimal" placeholder="z.B. 10"><span>mm²</span></span></label><label>Vorsicherung / Absicherung<input class="uv-feed-fuse" value="${esc(d.feedFuse||d.feed||'')}" placeholder="z.B. 35 A"></label><label>Ort<input class="uv-place" value="${esc(d.place||'')}"></label></div><div class="row" style="margin-top:10px"><button onclick="addRcd('${uvId}')">+ FI/RCD</button><button class="secondary" onclick="addCircuit(this.closest('.uv').querySelector('.directCircuits'))">+ Stromkreis ohne FI</button><button class="secondary" onclick="addCircuit(this.closest('.uv').querySelector('.directCircuits'),{device:'fils',name:'FI/LS Stromkreis'})">+ FI/LS</button></div><div class="subhead">FI/RCD-Gruppen</div><div class="rcds"></div><div class="subhead">Stromkreise ohne FI</div><div class="directCircuits circuits"></div></div></div>`}
...
function addRcd(uvId,data={}){const uv=document.querySelector(`[data-uv="${uvId}"]`);const rcdId=data.id||id();uv.querySelector('.rcds').insertAdjacentHTML('beforeend',rcdHtml(rcdId,data));const r=uv.querySelector(`[data-rcd="${rcdId}"]`);(data.circuits||[]).forEach(c=>addCircuit(r.querySelector('.circuits'),c));if(!(data.circuits||[]).length)addCircuit(r.querySelector('.circuits'));
evalAll();}
function rcdHtml(rcdId,d={}){return `<div class="rcd" data-rcd="${rcdId}"><div class="titleRow sectionHead"><button class="collapseBtn" onclick="toggleSection(this)">▾</button><h4>FI/RCD</h4><button class="danger small" onclick="this.closest('.rcd').remove();evalAll();saveData()">FI löschen</button></div><div class="rcdBody"><div class="grid"><label>Bezeichnung<input class="rcd-name" value="${esc(d.name||'FI')}" placeholder="z.B. FI Bad/Küche"></label><label>Typ<select class="rcd-type"><option ${d.type==='A'?'selected':''}>A</option><option ${d.type==='F'?'selected':''}>F</option><option ${d.type==='B'?'selected':''}>B</option><option ${d.type==='AC'?'selected':''}>AC</option></select></label><label>Charakteristik<select class="rcd-char"><option ${d.char==='unverzögert'?'selected':''}>unverzögert</option><option ${d.char==='G / kurzzeitverzögert'?'selected':''}>G / kurzzeitverzögert</option><option ${d.char==='S / selektiv'?'selected':''}>S / selektiv</option></select></label><label>Bemessungsstrom In<select class="rcd-in"><option>25 A</option><option ${d.inn==='40 A'?'selected':''}>40 A</option><option ${d.inn==='63 A'?'selected':''}>63 A</option><option ${d.inn==='80 A'?'selected':''}>80 A</option></select></label><label>Bemessungsdifferenzstrom IΔn<select class="rcd-idn"><option ${d.idn==='10'?'selected':''} value="10">10 mA</option><option ${!d.idn||d.idn==='30'?'selected':''} value="30">30 mA</option><option ${d.idn==='100'?'selected':''} value="100">100 mA</option><option ${d.idn==='300'?'selected':''} value="300">300 mA</option></select></label><label class="checkLabel">Prüftaste<input class="rcd-test" type="checkbox" ${d.test?'checked':''}></label></div><div class="status neutral rcd-status">FI-Bewertung fehlt</div><div class="row" style="margin-top:10px"><button onclick="addCircuit(this.closest('.rcd').querySelector('.circuits'))">+ Sicherung/Stromkreis nach FI</button></div><div class="circuits"></div></div></div>`}
function addCircuit(container,data={}){if(typeof container==='string')container=document.querySelector(container);data=data||{};data.insideRcd=!!container.closest('.rcd');if(data.insideRcd&&data.device==='fils')data.device='ls';container.insertAdjacentHTML('beforeend',circuitHtml(data));const el=container.lastElementChild;updateCircuitVisibility(el);evalAll();saveData();}
function moveCircuit(btn,dir){const c=btn.closest('.circuit');if(!c)return;if(dir<0&&c.previousElementSibling)c.parentElement.insertBefore(c,c.previousElementSibling);if(dir>0&&c.nextElementSibling)c.parentElement.insertBefore(c.nextElementSibling,c);evalAll();saveData();}
function circuitHtml(d={}){const dev=d.device||'ls';const inside=!!d.insideRcd;const deviceField=inside?`<div class="badgeField"><span>Schutzart</span><b>LS mit FI davor</b><input type="hidden" class="ck-device" value="ls"></div>`:`<label>Schutzorgan<select class="ck-device" onchange="updateCircuitVisibility(this.closest('.circuit'))"><option value="ls" ${dev!=='fils'?'selected':''}>LS ohne FI</option><option value="fils" ${dev==='fils'?'selected':''}>FI/LS</option></select></label>`;return `<div class="circuit ${inside?'insideRcd':''}"><div class="circuitTop"><b>Stromkreis</b><span class="hint">Reihenfolge entspricht dem Verteiler</span><div class="moveBtns"><button type="button" class="secondary small" onclick="moveCircuit(this,-1)">↑</button><button type="button" class="secondary small" onclick="moveCircuit(this,1)">↓</button></div></div><div class="grid">${deviceField}<label>Stromkreis / Sicherung<input class="ck-name" value="${esc(d.name||'')}" placeholder="z.B. F1 Steckdosen Küche"></label><label>Kabeltyp / Leitung${cableSelect('ck-cable', d.cable||'')}</label><label>Adernzahl${coreSelect('ck-cores', d.cores||'')}</label><label>Querschnitt<span class="unitField"><input class="ck-cross" value="${esc(d.cross||'')}" inputmode="decimal" placeholder="z.B. 1,5"><span>mm²</span></span></label><label>LS-Charakteristik<select class="ck-char"><option ${d.char==='B'?'selected':''}>B</option><option ${d.char==='C'?'selected':''}>C</option><option ${d.char==='D'?'selected':''}>D</option></select></label><label>LS-Nennstrom<select class="ck-a"><option>6</option><option ${d.a==='10'?'selected':''}>10</option><option ${d.a==='13'?'selected':''}>13</option><option ${!d.a||d.a==='16'?'selected':''}>16</option><option ${d.a==='20'?'selected':''}>20</option><option ${d.a==='25'?'selected':''}>25</option><option ${d.a==='32'?'selected':''}>32</option><option ${d.a==='40'?'selected':''}>40</option></select></label><label class="filsOnly">FI/LS Typ<select class="ck-rcdtype"><option ${!d.rcdType||d.rcdType==='A'?'selected':''}>A</option><option ${d.rcdType==='F'?'selected':''}>F</option><option ${d.rcdType==='B'?'selected':''}>B</option><option ${d.rcdType==='AC'?'selected':''}>AC</option></select></label><label class="filsOnly">FI/LS Charakteristik<select class="ck-rcdchar"><option ${!d.rcdChar||d.rcdChar==='unverzögert'?'selected':''}>unverzögert</option><option ${d.rcdChar==='G / kurzzeitverzögert'?'selected':''}>G / kurzzeitverzögert</option><option ${d.rcdChar==='S / selektiv'?'selected':''}>S / selektiv</option></select></label><label class="filsOnly">FI/LS IΔn<select class="ck-rcdidn"><option ${d.rcdIdn==='10'?'selected':''} value="10">10 mA</option><option ${!d.rcdIdn||d.rcdIdn==='30'?'selected':''} value="30">30 mA</option><option ${d.rcdIdn==='100'?'selected':''} value="100">100 mA</option><option ${d.rcdIdn==='300'?'selected':''} value="300">300 mA</option></select></label><label class="checkLabel filsOnly">FI/LS Prüftaste<input class="ck-rcdtest" type="checkbox" ${d.rcdTest?'checked':''}></label><label>RPE Ω<input class="ck-rpe" value="${esc(d.rpe||'')}" inputmode="decimal"></label><label>RISO MΩ<input class="ck-riso" value="${esc(d.riso||'')}" inputmode="decimal"></label><label>Zi Ω<input class="ck-zi" value="${esc(d.zi||'')}" inputmode="decimal" placeholder="Netzinnenimpedanz"></label><label>Zs Ω<input class="ck-zs" value="${esc(d.zs||'')}" inputmode="decimal" placeholder="Schleifenimpedanz"></label><label>IK A<input class="ck-ik" value="${esc(d.ik||'')}" inputmode="decimal"></label><label class="rcdMeasure">RCD Auslösezeit ms<input class="ck-rcdms" value="${esc(d.rcdms||'')}" inputmode="decimal" placeholder="nur bei FI/FI-LS"></label><label class="rcdMeasure">RCD Auslösestrom mA<input class="ck-rcdma" value="${esc(d.rcdma||'')}" inputmode="decimal" placeholder="nur bei FI/FI-LS"></label><label>Bemerkung<input class="ck-note" value="${esc(d.note||'')}"></label></div><div class="status neutral">Bewertung fehlt</div><button class="danger small" onclick="this.closest('.circuit').remove();evalAll();saveData()">Stromkreis löschen</button></div>`}
...
function loadData(data={}){
  ...
  fields.forEach(id=>{const e=document.getElementById(id);if(e&&source.fields[id]!==undefined){if(e.type==='checkbox')e.checked=!!source.fields[id];else e.value=source.fields[id]||''}});
  ...
  updateLogoPreview();evalAll();suppressAutosave=false;
}
...
document.addEventListener('input',e=>{if(e.target.closest('#appMain')){evalAll();saveData()}});
document.addEventListener('change',e=>{if(e.target.closest('#appMain')){evalAll();saveData()}});
...
function preparePrint(){
  ...
  (d.uvs||[]).forEach((u,ui)=>{
    ...
    (u.rcds||[]).forEach((r,ri)=>{
      ...
      rows += `<tr><td colspan="19"><b>FI/RCD:</b> ${esc(r.name||('FI '+(ri+1)))} | Typ ${esc(r.type)} | Charakteristik ${esc(r.char)} | In ${esc(r.inn)} | IΔn ${esc(r.idn)} mA | Prüftaste ${yn(r.test)} | <span class="${re.state}">${re.text}</span>${re.msg?' – '+esc(re.msg):''}</td></tr>`;
      rows += measHeader();
      (r.circuits||[]).forEach((c,ci)=>{const ce=circuitEvalText(c,true,r); if(ce.state==='bad')bad++; else if(ce.state==='warn')warn++; rows += measRow(u.name,r.name,c,ce,ci+1,true,r);});
    });
    if((u.direct||[]).length){
      rows += `<tr><td colspan="19"><b>Direkte Stromkreise: LS ohne FI davor oder FI/LS integriert</b></td></tr>`+measHeader();
      (u.direct||[]).forEach((c,ci)=>{const ce=circuitEvalText(c,c.device==='fils',null); if(ce.state==='bad')bad++; else if(ce.state==='warn')warn++; rows += measRow(u.name,c.device==='fils'?'FI/LS integriert':'ohne FI/RCD',c,ce,ci+1,false,null);});
    }
  });
  ...
  document.getElementById('printReport').innerHTML = `
    <div class="printHeader"><div>${logo}</div><div><h1>Elektro-Prüfprotokoll</h1><p>DIN VDE 0100-600 / DIN VDE 0105-100 / DGUV Vorschrift 3</p></div><div class="smalltxt"><b>Datum:</b> ${esc(f.datum||'-')}<br><b>Prüfer:</b> ${esc(f.pruefer||'-')}</div></div>
    <div class="meta">
...
function captureInitialFieldValues(){
  fields.forEach(id=>{const e=document.getElementById(id);initialFieldValues[id]=e.type==='checkbox'?false:e.value});
}
...
function openProtocolDialog(){
  if(!currentSiteId)return;document.getElementById('protocolForm').reset();document.getElementById('newProtocolDate').value=new Date().toISOString().slice(0,10);showDialog(document.getElementById('protocolDialog'));setTimeout(()=>document.getElementById('newProtocolTitle').focus(),50);
}
...
async function openProtocol(protocolId){
  ...
  loadData(protocol.data||makeBlankData());setSaveState('saved','Gespeichert');window.scrollTo({top:0,behavior:'smooth'});
}
...
async function persistCurrentProtocol(manual=false){
  ...
  try{
    ...
    const company={};firmFields.forEach(id=>company[id]=data.fields[id]||'');await dbPut('settings',{key:'company',data:company,updatedAt:time});
