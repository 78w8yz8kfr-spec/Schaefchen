...
fields.push('masterName');
const checkGroups=[
  {title:'Besichtigen',items:['Auswahl der Betriebsmittel','Trenn- und Schaltgeräte','Brandabschottungen','Gebäudesystemtechnik','Kabel, Leitungen und Stromschienen','Kennzeichnung der Stromkreise und Betriebsmittel','Kennzeichnung von N- und PE-Leitern','Leiterverbindungen','Schutz-, Sicherheits- und Überwachungseinrichtungen','Basisschutz (Schutz gegen direktes Berühren)','Zugänglichkeit von HAK und Verteilern','Schutzpotentialausgleich','Zusätzlicher Schutzpotentialausgleich','Funktionspotentialausgleich','Dokumentation vollständig']},
  {title:'Erproben',items:['Funktionsprüfung der Anlage','FI-Schutzschalter (RCD)','Funktion der Schutz-, Sicherheits- und Überwachungseinrichtungen','Rechtsdrehfeld (Drehstromsteckdosen)','Spannungsfall überprüft','Gebäudesystemtechnik erprobt','Spannungspolarität','Abschaltbedingungen / automatische Abschaltung']}
];
const checkItems=checkGroups.flatMap(group=>group.items);
const potentialFields=[['paFoundation','Fundamenterder'],['paRing','Ringerder'],['paHes','Haupterdungsschiene'],['paWaterMeter','Wasserzwischenzähler'],['paMainWater','Hauptwasserleitung'],['paMainPe','Hauptschutzleiter'],['paGas','Gasinnenleitung'],['paHeating','Heizungsanlage'],['paClimate','Klimaanlage'],['paLift','Aufzugsanlage'],['paEdv','EDV-Anlage'],['paPhone','Telefonanlage'],['paLightning','Blitzschutzanlage'],['paAntenna','Antennenanlage / BK'],['paBuilding','Gebäudekonstruktion']];
const fuses={
...
let currentStep=1;
const firmFields=['firmName','firmStreet','firmCity','firmTel','firmFax','firmMobile','firmEmail','firmCEO','firmCourt','firmHRB','defaultPruefer','masterName'];
const deviceFields=['gVde','gHersteller','gTyp','gSerie','gKal','g2Hersteller','g2Typ','g2Serie','g2Kal','g3Hersteller','g3Typ','g3Serie','g3Kal'];
...
const checks=document.getElementById('checks');let checkIndex=0;
checkGroups.forEach(group=>{checks.insertAdjacentHTML('beforeend',`<h3 class="checkGroupTitle">${group.title}</h3>`);group.items.forEach(t=>{const i=checkIndex++;checks.insertAdjacentHTML('beforeend',`<div class="checkrow"><span>${t}</span><label><input type="radio" name="c${i}" value="io"> i.O.</label><label><input type="radio" name="c${i}" value="nio"> n.i.O.</label></div>`)});});
...
function deDate(value){if(!value)return '—';const match=String(value).match(/^(\d{4})-(\d{2})-(\d{2})/);return match?`${match[3]}.${match[2]}.${match[1]}`:String(value)}
function deNumber(value){if(value===undefined||value===null||String(value).trim()==='')return '—';return String(value).trim().replace(/\s+/g,' ').replace(/(\d)\.(\d)/g,'$1,$2')}
function pdfSafeText(value){return String(value===undefined||value===null?'':value).replace(/MΩ/g,'MOhm').replace(/Ω/g,'Ohm').replace(/IΔn/g,'Idn')}
function deviceCalibrationIssues(source){const f=source||{},today=new Date();today.setHours(0,0,0,0);const result=[];[[f.gHersteller,f.gTyp,f.gKal,1],[f.g2Hersteller,f.g2Typ,f.g2Kal,2],[f.g3Hersteller,f.g3Typ,f.g3Kal,3]].forEach(([manufacturer,type,date,index])=>{if(!manufacturer&&!type&&!date)return;if(!date){result.push({text:`Kalibrierungsdatum für Messgerät ${index} fehlt`,critical:false});return}const due=new Date(`${date}T00:00:00`);if(Number.isNaN(due.getTime())){result.push({text:`Kalibrierungsdatum für Messgerät ${index} ist ungültig`,critical:true});return}const days=Math.ceil((due-today)/86400000);if(days<0)result.push({text:`Kalibrierung von Messgerät ${index} ist seit ${deDate(date)} abgelaufen`,critical:true});else if(days<=30)result.push({text:`Kalibrierung von Messgerät ${index} läuft am ${deDate(date)} ab`,critical:false})});return result}
function updateCalibrationStatus(){const box=document.getElementById('calibrationStatus');if(!box)return;const source={};deviceFields.forEach(id=>source[id]=document.getElementById(id)?.value||'');const hasDevice=deviceFields.some(id=>id!=='gVde'&&document.getElementById(id)?.value.trim()),issues=deviceCalibrationIssues(source);box.className='calibrationStatus';if(!hasDevice){box.classList.add('show','warn');box.textContent='Prüfgerät noch nicht vollständig voreingestellt.'}else if(issues.length){box.classList.add('show',issues.some(item=>item.critical)?'bad':'warn');box.textContent=issues.map(item=>item.text).join(' · ')}else{box.classList.add('show','ok');box.textContent='Prüfgerät und Kalibrierung sind erfasst.'}}
function addUv(data={}){const uvId=data.id||id();document.getElementById('uvs').insertAdjacentHTML('beforeend',uvHtml(uvId,data));const uv=document.querySelector(`[data-uv="${uvId}"]`);(data.rcds||[]).forEach(r=>addRcd(uvId,r));(data.direct||[]).forEach(c=>addCircuit(uv.querySelector('.directCircuits'),c));if(!(data.rcds||[]).length&&!(data.direct||[]).length)addRcd(uvId,{name:'FI 1'});ensureCircuitBatchControl(uv,'direct');updateStructureLabels();evalAll();}
...
function addRcd(uvId,data={}){const uv=document.querySelector(`[data-uv="${uvId}"]`);const rcdId=data.id||id();uv.querySelector('.rcds').insertAdjacentHTML('beforeend',rcdHtml(rcdId,data));const r=uv.querySelector(`[data-rcd="${rcdId}"]`);(data.circuits||[]).forEach(c=>addCircuit(r.querySelector('.circuits'),c));if(!(data.circuits||[]).length)addCircuit(r.querySelector('.circuits'));ensureCircuitBatchControl(r,'rcd');updateStructureLabels();evalAll();}
...
function setCircuitHeading(c,number){const index=c.querySelector('.circuit-index'),title=c.querySelector('.circuit-title');if(index)index.textContent=`Stromkreis ${number}`;if(title)title.textContent=val(c,'.ck-name')||'Noch nicht bezeichnet';updateCircuitSummary(c)}
...
const risoKeys=['risoNpe','risoL1pe','risoL1n','risoL2pe','risoL2n','risoL3pe','risoL3n','risoL1l2','risoL1l3','risoL2l3'];
function effectiveRisoElement(c){const summary=dec(val(c,'.ck-riso'));if(!isNaN(summary))return summary;const values=['.ck-riso-npe','.ck-riso-l1pe','.ck-riso-l1n','.ck-riso-l2pe','.ck-riso-l2n','.ck-riso-l3pe','.ck-riso-l3n','.ck-riso-l1l2','.ck-riso-l1l3','.ck-riso-l2l3'].map(s=>dec(val(c,s))).filter(v=>!isNaN(v));return values.length?Math.min(...values):NaN}
function effectiveRisoData(c){const summary=dec(c.riso);if(!isNaN(summary))return summary;const values=risoKeys.map(key=>dec(c[key])).filter(v=>!isNaN(v));return values.length?Math.min(...values):NaN}
function evalCircuit(c,hasRcd){const isFils=val(c,'.ck-device')==='fils';hasRcd=hasRcd||isFils;const riso=effectiveRisoElement(c), zi=dec(val(c,'.ck-zi')), zs=dec(val(c,'.ck-zs')), ik=dec(val(c,'.ck-ik')), rpe=dec(val(c,'.ck-rpe'));const key=val(c,'.ck-char')+val(c,'.ck-a');const ia=fuses[key];let ok=true,warn=false,msg=[];
 ...
 const st=c.querySelector('.status');st.className='status '+(ok?(warn?'warn':'ok'):'bad');st.textContent=ok?(warn?'Stromkreis: prüfen / Angaben unvollständig':'Stromkreis: i.O.'):'Stromkreis: n.i.O.';if(msg.length)st.innerHTML+=`<ul class="details"><li>${msg.map(esc).join('</li><li>')}</li></ul>`;updateCircuitSummary(c);return {ok,warn,msg};}
...
function collect(){const data={schemaVersion:20,fields:{},checks:{},uvs:[],photos:currentPhotos.map(p=>({...p})),sig:document.getElementById('sig').toDataURL()};fields.forEach(id=>{const e=document.getElementById(id);if(!e)return;data.fields[id]=(e.type==='checkbox'||e.type==='radio')?e.checked:e.value});checkItems.forEach((_,i)=>{const e=document.querySelector(`[name=c${i}]:checked`);data.checks[i]=e?e.value:''});document.querySelectorAll('.uv').forEach(uv=>{const u={id:uv.dataset.uv,name:val(uv,'.uv-name'),source:val(uv,'.uv-source'),feedCable:cableFieldValue(uv,'.uv-feed-cable','.uv-feed-cable-custom'),feedCores:val(uv,'.uv-feed-cores'),feedCross:val(uv,'.uv-feed-cross'),feedFuse:val(uv,'.uv-feed-fuse'),feed:val(uv,'.uv-feed-fuse'),place:val(uv,'.uv-place'),rcds:[],direct:[]};uv.querySelectorAll(':scope .uvBody > .rcds > .rcd').forEach(r=>{const rd={id:r.dataset.rcd,name:val(r,'.rcd-name'),type:val(r,'.rcd-type'),char:val(r,'.rcd-char'),inn:val(r,'.rcd-in'),idn:val(r,'.rcd-idn'),test:checked(r,'.rcd-test'),circuits:[]};r.querySelectorAll('.circuit').forEach(c=>rd.circuits.push(circuitData(c)));u.rcds.push(rd)});uv.querySelectorAll(':scope .uvBody > .directCircuits > .circuit').forEach(c=>u.direct.push(circuitData(c)));data.uvs.push(u)});return data}
function circuitData(c){const inRcd=!!c.closest('.rcd');const device=inRcd?'ls':(val(c,'.ck-device')||'ls');return {device,name:val(c,'.ck-name'),cable:cableFieldValue(c,'.ck-cable','.ck-cable-custom'),cores:val(c,'.ck-cores'),cross:val(c,'.ck-cross'),char:val(c,'.ck-char'),a:val(c,'.ck-a'),rcdType:val(c,'.ck-rcdtype'),rcdChar:val(c,'.ck-rcdchar'),rcdIdn:val(c,'.ck-rcdidn'),rcdTest:checked(c,'.ck-rcdtest'),rpe:val(c,'.ck-rpe'),riso:val(c,'.ck-riso'),risoV:val(c,'.ck-risov'),consumer:val(c,'.ck-consumer'),risoNpe:val(c,'.ck-riso-npe'),risoL1pe:val(c,'.ck-riso-l1pe'),risoL1n:val(c,'.ck-riso-l1n'),risoL2pe:val(c,'.ck-riso-l2pe'),risoL2n:val(c,'.ck-riso-l2n'),risoL3pe:val(c,'.ck-riso-l3pe'),risoL3n:val(c,'.ck-riso-l3n'),risoL1l2:val(c,'.ck-riso-l1l2'),risoL1l3:val(c,'.ck-riso-l1l3'),risoL2l3:val(c,'.ck-riso-l2l3'),zi:val(c,'.ck-zi'),zs:val(c,'.ck-zs'),ik:val(c,'.ck-ik'),rcdUl:val(c,'.ck-rcdul'),rcdms:val(c,'.ck-rcdms'),rcdma:val(c,'.ck-rcdma'),note:val(c,'.ck-note')}}
const baseCircuitData=circuitData;
circuitData=function(c){const data=baseCircuitData(c);data.defectPriority=val(c,'.ck-defect-priority');data.defect=val(c,'.ck-defect');data.defectPhotos=(c._defectPhotos||[]).map(photo=>({...photo}));return data};
function saveData(manual=false){
  if(suppressAutosave||!currentProtocolId||document.getElementById('appMain')?.classList.contains('isLocked'))return;
...
function loadData(data={}){
  ...
  if(!data.schemaVersion||data.schemaVersion<20){const legacyMap={0:9,1:11,2:0,3:5,4:16,5:18,6:21,7:22},migrated={};Object.entries(source.checks||{}).forEach(([oldIndex,value])=>{const target=legacyMap[oldIndex];if(target!==undefined)migrated[target]=value});source.checks=migrated}
  ...
  updateLogoPreview();updateStructureLabels();evalAll();updateCalibrationStatus();suppressAutosave=false;showStep(1,false);
}
...
function completionIssues(){
  const issues=[],add=(text,step,critical=false)=>issues.push({text,step,critical});[['kunde','Auftraggeber'],['objekt','Anlage / Objekt'],['datum','Beginn der Prüfung'],['pruefer','Prüfer']].forEach(([id,label])=>{if(!document.getElementById(id).value.trim())add(`${label} fehlt`,1,true)});
  if(!document.querySelector('[name=pruefart]:checked'))add('Art der Prüfung fehlt',1,true);
  if(!deviceFields.some(id=>document.getElementById(id)?.value.trim()))add('Prüfgerät fehlt',1,true);
  const currentFields={};deviceFields.forEach(id=>currentFields[id]=document.getElementById(id)?.value||'');deviceCalibrationIssues(currentFields).forEach(item=>add(item.text,1,item.critical));
  const unrated=checkItems.filter((_,i)=>!document.querySelector(`[name=c${i}]:checked`)).length;if(unrated)add(`${unrated} Sicht-/Funktionsprüfungen sind unbewertet`,2,false);
  const circuits=[...document.querySelectorAll('.circuit')];if(!circuits.length)add('Kein Stromkreis angelegt',4,true);else{const incomplete=circuits.filter(c=>!val(c,'.ck-name')||isNaN(effectiveRisoElement(c))).length;if(incomplete)add(`${incomplete} Stromkreis${incomplete===1?' ist':'e sind'} unvollständig`,4,true)}
  const hasBadCheck=!!document.querySelector('#checks input[value="nio"]:checked'),hasBadCircuit=circuits.some(c=>c.querySelector('.status')?.classList.contains('bad')),hasAssignedDefect=circuits.some(c=>val(c,'.ck-defect')||val(c,'.ck-defect-priority')),result=document.getElementById('ergebnis').value||'';if((hasBadCheck||hasBadCircuit||hasAssignedDefect)&&result==='Anlage ist in Ordnung')add('Ergebnis „Anlage ist in Ordnung“ widerspricht festgestellten Fehlern oder Mängeln',5,true);
  if(!document.getElementById('plakette').value)add('Prüfplakette ist nicht bewertet',5,false);
  if(!document.getElementById('next').value)add('Nächster Prüftermin fehlt',5,false);
  return issues;
}
async function completeProtocol(){
  if(!currentProtocolId)return;const issues=completionIssues();
  if(issues.length){showCompletionDialog(issues);return}
  await finalizeProtocolCompletion();
}
function showCompletionDialog(issues){const list=document.getElementById('completionIssueList'),critical=issues.filter(item=>item.critical).length;document.getElementById('completionSummary').textContent=critical?`${critical} wichtige und ${issues.length-critical} weitere offene Angaben wurden gefunden.`:`${issues.length} offene Angaben wurden gefunden.`;list.innerHTML=issues.map(item=>`<div class="completionIssue ${item.critical?'critical':''}"><span class="issueMark">${item.critical?'⚠':'ℹ'}</span><span>${esc(item.text)}</span><button type="button" class="secondary" onclick="goToCompletionIssue(${item.step})">Schritt ${item.step}</button></div>`).join('');document.getElementById('confirmCompletionButton').textContent=critical?'Bewusst trotzdem abschließen':'Trotzdem abschließen';showDialog(document.getElementById('completionDialog'))}
function closeCompletionDialog(){closeDialog(document.getElementById('completionDialog'))}
function goToCompletionIssue(step){closeCompletionDialog();showStep(step)}
async function confirmProtocolCompletion(){closeCompletionDialog();await finalizeProtocolCompletion()}
async function finalizeProtocolCompletion(){
  const now=new Date();if(!document.getElementById('endDate').value)document.getElementById('endDate').value=now.toISOString().slice(0,10);if(!document.getElementById('endTime').value)document.getElementById('endTime').value=now.toTimeString().slice(0,5);
  document.getElementById('protocolStatus').value='completed';await persistCurrentProtocol(false);const record=await dbGet('protocols',currentProtocolId);if(record){record.completedAt=nowIso();record.updatedAt=record.completedAt;await dbPut('protocols',record);currentProtocolRecord=record}showToast('Prüfung abgeschlossen und gespeichert');setTimeout(()=>closeEditor(),500);
}
function setEditorLocked(locked){const app=document.getElementById('appMain'),banner=document.getElementById('lockedBanner');app.classList.toggle('isLocked',locked);banner.classList.toggle('hidden',!locked);if(locked){const stamp=currentProtocolRecord&&(currentProtocolRecord.completedAt||currentProtocolRecord.updatedAt);document.getElementById('lockedBannerText').textContent=`Schreibgeschützt${stamp?' · abgeschlossen '+formatDate(stamp,true):''}. Für Änderungen zuerst erneut bearbeiten.`}app.querySelectorAll('input,select,textarea').forEach(element=>element.disabled=locked);app.querySelectorAll('button').forEach(button=>{const allowed=button.matches('#editorBackButton,#reopenButton,#pdfButton,#printButton,#prevStep,#nextStep,[data-step-target]');button.disabled=locked&&!allowed});canvas.style.pointerEvents=locked?'none':''}
async function reopenProtocolForEditing(){if(!currentProtocolId||!confirm('Diese abgeschlossene Prüfung wirklich erneut zur Bearbeitung öffnen?'))return;const record=await dbGet('protocols',currentProtocolId);if(!record)return;record.status='draft';record.reopenedAt=nowIso();record.updatedAt=record.reopenedAt;await dbPut('protocols',record);currentProtocolRecord=record;document.getElementById('protocolStatus').value='draft';setEditorLocked(false);showToast('Prüfung ist wieder bearbeitbar')}
document.addEventListener('input',e=>{if(e.target.closest('#appMain')){if(e.target.matches('.uv-name,.rcd-name,.ck-name'))updateStructureLabels();evalAll();updateCalibrationStatus();updateStepProgress();saveData()}});
document.addEventListener('change',e=>{if(e.target.closest('#appMain')){if(e.target.id==='protocolStatus'&&e.target.value==='completed'){e.target.value='draft';completeProtocol();return}evalAll();updateCalibrationStatus();updateStepProgress();saveData()}});
...
function circuitEvalText(c,hasRcd,r){
  ...
  const riso=effectiveRisoData(c), zi=dec(c.zi), zs=dec(c.zs), ik=dec(c.ik), rpe=dec(c.rpe);
...
function compactCheckRows(checkData={}){let index=0;const groups=checkGroups.map(group=>group.items.map(item=>{const value=checkData&&checkData[index++];return [item,value==='io'?'i.O.':value==='nio'?'n.i.O.':'—']})),besichtigen=groups[0]||[],erproben=groups[1]||[],split=Math.ceil(besichtigen.length/2),left=besichtigen.slice(0,split),middle=besichtigen.slice(split),count=Math.max(left.length,middle.length,erproben.length),rows=[];for(let i=0;i<count;i++){rows.push([left[i]?.[0]||'',left[i]?.[1]||'',middle[i]?.[0]||'',middle[i]?.[1]||'',erproben[i]?.[0]||'',erproben[i]?.[1]||''])}return rows}
function protocolCircuitEntries(d){const entries=[];(d.uvs||[]).forEach((u,ui)=>{(u.rcds||[]).forEach((r,ri)=>(r.circuits||[]).forEach((c,ci)=>entries.push({number:`${ui+1}.${ri+1}.${ci+1}`,uv:u.name||`UV ${ui+1}`,fi:r.name||`FI ${ri+1}`,c})));(u.direct||[]).forEach((c,ci)=>entries.push({number:`${ui+1}.D${ci+1}`,uv:u.name||`UV ${ui+1}`,fi:c.device==='fils'?'FI/LS integriert':'—',c}))});return entries}
function defectPriorityLabel(value){return value==='hoch'?'Hoch':value==='mittel'?'Mittel':value==='hinweis'?'Hinweis':'—'}
function reportPhotos(d){const photos=(d.photos||[]).map(photo=>({...photo,caption:photo.name||'Allgemeines Foto'}));protocolCircuitEntries(d).forEach(entry=>(entry.c.defectPhotos||[]).forEach(photo=>photos.push({...photo,caption:`Stromkreis ${entry.number} · ${entry.c.name||'ohne Bezeichnung'} · ${photo.name||'Foto'}`})));return photos}
function preparePrint(){
  evalAll(); saveData();
  const d=collect(); const f=d.fields||{};
  let bad=0,warn=0,rows='',insulationRows='';
  (d.uvs||[]).forEach((u,ui)=>{
    ...
    (u.rcds||[]).forEach((r,ri)=>{
      ...
      rows += measHeader();
      (r.circuits||[]).forEach((c,ci)=>{const number=`${fiNumber}.${ci+1}`,ce=circuitEvalText(c,true,r);if(ce.state==='bad')bad++;else if(ce.state==='warn')warn++;rows+=measRow(u.name,r.name,c,ce,number,true,r);insulationRows+=insulationRow(number,c)});
    });
    if((u.direct||[]).length){
      rows += `<tr><td colspan="20"><b>Direkte Schutzorgane: LS und FI/LS</b></td></tr>`+measHeader();
      (u.direct||[]).forEach((c,ci)=>{const number=`${ui+1}.D${ci+1}`,ce=circuitEvalText(c,c.device==='fils',null);if(ce.state==='bad')bad++;else if(ce.state==='warn')warn++;rows+=measRow(u.name,c.device==='fils'?'FI/LS integriert':'ohne FI/RCD',c,ce,number,false,null);insulationRows+=insulationRow(number,c)});
    }
  });
  const checksHtml=compactCheckRows(d.checks).map(row=>`<tr>${row.map((value,index)=>`<td class="${index%2?'checkValue':''}">${esc(value)}</td>`).join('')}</tr>`).join('');
  const prueferSig = d.sig && d.sig.length>1000 ? `<img src="${d.sig}">` : '',entries=protocolCircuitEntries(d),defects=entries.filter(entry=>entry.c.defect||entry.c.defectPriority),defectsHtml=defects.length?`<h2>Stromkreisbezogene Mängel</h2><table><thead><tr><th>Nr.</th><th>UV</th><th>Stromkreis</th><th>Priorität</th><th>Feststellung / Maßnahme</th><th>Fotos</th></tr></thead><tbody>${defects.map(entry=>`<tr><td>${entry.number}</td><td>${esc(entry.uv)}</td><td>${esc(entry.c.name||'-')}</td><td>${defectPriorityLabel(entry.c.defectPriority)}</td><td>${esc(entry.c.defect||'-')}</td><td>${(entry.c.defectPhotos||[]).length}</td></tr>`).join('')}</tbody></table>`:'',allPhotos=reportPhotos(d);
  const photosHtml=allPhotos.length?`<h2>Fotodokumentation</h2><div class="printPhotos">${allPhotos.map((photo,i)=>`<figure><img src="${photo.dataUrl}" alt="Foto ${i+1}"><figcaption>${esc(photo.caption||photo.name||`Foto ${i+1}`)}</figcaption></figure>`).join('')}</div>`:'';
  ...
  const potential=potentialFields.filter(([id])=>f[id]).map(([,label])=>label).concat(f.paOther?[f.paOther]:[]).join(', ')||'Keine Angaben';
  const devices=[[f.gHersteller,f.gTyp,f.gSerie,f.gKal],[f.g2Hersteller,f.g2Typ,f.g2Serie,f.g2Kal],[f.g3Hersteller,f.g3Typ,f.g3Serie,f.g3Kal]].filter(row=>row.some(Boolean)).map((row,i)=>`<b>Messgerät ${i+1}:</b> ${esc(row[0])} ${esc(row[1])} · SN ${esc(row[2]||'-')} · Kalibrierung ${esc(deDate(row[3]))}`).join('<br>')||'Keine Angaben';
  ...
  document.getElementById('printReport').innerHTML = `
    ...
    <h2>Detaillierte Isolationsmessung</h2>
    <table><tr><th>Nr.</th><th>U Mess V</th><th>Verbraucher</th><th>N–PE</th><th>L1–PE</th><th>L1–N</th><th>L2–PE</th><th>L2–N</th><th>L3–PE</th><th>L3–N</th><th>L1–L2</th><th>L1–L3</th><th>L2–L3</th></tr>${insulationRows||'<tr><td colspan="13">Keine Detailwerte eingetragen</td></tr>'}</table>
...
async function createPdf(){
  ...
  try{
    ...
    const {jsPDF}=window.jspdf,doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4',compress:true}),d=collect(),f=d.fields||{},pageW=doc.internal.pageSize.getWidth(),pageH=doc.internal.pageSize.getHeight(),margin=10;
    ...
    const table=(head,body,options={})=>{runTable({startY:y,head:[head],body,theme:'grid',showHead:'everyPage',rowPageBreak:'avoid',margin:{left:margin,right:margin},styles:{font:'helvetica',fontSize:options.fontSize||7.5,cellPadding:1.4,valign:'middle',overflow:'linebreak'},headStyles:{fillColor:[192,24,32],textColor:255,fontStyle:'bold'},alternateRowStyles:{fillColor:[247,249,251]},...options});y=(doc.lastAutoTable&&doc.lastAutoTable.finalY||y)+5};
    ...
    table(['Angabe','Wert','Angabe','Wert'],[
      ['Kunden-Nr.',f.customerNo||'-','Auftrags-Nr.',f.orderNo||'-'],['Auftraggeber',f.kunde||'-','Auftragnehmer',f.firmName||f.firma||'-'],['Anlage / Objekt',f.objekt||'-','Adresse',f.adresse||'-'],['Beginn',`${deDate(f.datum)} ${f.startTime||''}`.trim(),'Ende',`${deDate(f.endDate)} ${f.endTime||''}`.trim()],['Prüfart',testType,'Regelwerk',standards],['Netz',`${f.spannung||'-'} · ${f.frequenz||'-'}`,'Netzsystem / Betreiber',`${f.netzform||'-'} · ${f.netzbetreiber||'-'}`]
    ],{columnStyles:{0:{cellWidth:30,fontStyle:'bold'},1:{cellWidth:65},2:{cellWidth:30,fontStyle:'bold'},3:{cellWidth:65}},fontSize:7});
    section('Verwendete Messgeräte');
    const deviceRows=[[f.gHersteller,f.gTyp,f.gSerie,f.gKal],[f.g2Hersteller,f.g2Typ,f.g2Serie,f.g2Kal],[f.g3Hersteller,f.g3Typ,f.g3Serie,f.g3Kal]].filter(row=>row.some(Boolean)).map((row,i)=>[i+1,row[0]||'-',row[1]||'-',row[2]||'-',deDate(row[3])]);
    ...
    section('Besichtigen und Erproben');const checkRows=compactCheckRows(d.checks);
    table(['Besichtigen','Bew.','Besichtigen','Bew.','Erproben','Bew.'],checkRows,{styles:{font:'helvetica',fontSize:6.5,cellPadding:1,valign:'middle',overflow:'linebreak'},columnStyles:{1:{cellWidth:14,halign:'center'},3:{cellWidth:14,halign:'center'},5:{cellWidth:14,halign:'center'}},fontSize:6.5});
    ...
    table(['Angabe','Wert'],[['Hausanschluss / Ort',`${f.hakName||'-'} · ${f.hakPlace||'-'}`],['Vorsicherung / Herkunft',`${f.hakFuse||'-'} · ${f.hakSource||'-'}`],['Zuleitung',`${displayCable(f.hakCable,f.hakCableCustom)||'-'} · ${f.hakCores||'-'}-adrig · ${deNumber(f.hakCross)} mm²`],['Spannungsfall',`${deNumber(f.voltageDrop)} %`],['Erdungswiderstand RE',`${deNumber(f.earthResistance)} Ohm`],['Potentialausgleich',`${deNumber(f.paResult)} Ohm`],['Einbezogene Systeme',potential]],{columnStyles:{0:{cellWidth:52,fontStyle:'bold'}},fontSize:8});
    const protectionRows=[],measurementRows=[],isolationRows=[];
    (d.uvs||[]).forEach((u,ui)=>{(u.rcds||[]).forEach((r,ri)=>{const fiNumber=`${ui+1}.${ri+1}`;(r.circuits||[]).forEach((c,cki)=>pdfCircuitRows(`${fiNumber}.${cki+1}`,u,r,c,true,protectionRows,measurementRows,isolationRows))});(u.direct||[]).forEach((c,cki)=>pdfCircuitRows(`${ui+1}.D${cki+1}`,u,null,c,false,protectionRows,measurementRows,isolationRows))});const defectRows=protocolCircuitEntries(d).filter(entry=>entry.c.defect||entry.c.defectPriority).map(entry=>[entry.number,entry.uv,entry.c.name||'-',defectPriorityLabel(entry.c.defectPriority),entry.c.defect||'-',String((entry.c.defectPhotos||[]).length)]);
    doc.addPage();y=12;
    section('Stromkreise und Schutzorgane');table(['Nr.','UV','Zielbezeichnung','Leitung','Adern / mm²','Schutzart','FI-Zuordnung','Schutzorgan','Bewertung'],protectionRows.length?protectionRows:[['-','-','Keine Stromkreise','-','-','-','-','-','-']],{columnStyles:{0:{cellWidth:12},1:{cellWidth:18},2:{cellWidth:34},3:{cellWidth:18},4:{cellWidth:20},5:{cellWidth:16},6:{cellWidth:24},7:{cellWidth:30},8:{cellWidth:18}},fontSize:5.5});
    section('Messwerte');table(['Nr.','RPE Ohm','RISO MOhm','U Mess V','Zi L-N Ohm','Zs L-PE Ohm','IK A','UL V','RCD ms','RCD mA','Bemerkung'],measurementRows.length?measurementRows:[['-','-','-','-','-','-','-','-','-','-','-']],{columnStyles:{0:{cellWidth:12},1:{cellWidth:12},2:{cellWidth:12},3:{cellWidth:12},4:{cellWidth:12},5:{cellWidth:12},6:{cellWidth:12},7:{cellWidth:12},8:{cellWidth:12},9:{cellWidth:12},10:{cellWidth:70}},fontSize:5.3});
    section('Detaillierte Isolationsmessung');table(['Nr.','Verbr.','N–PE','L1–PE','L1–N','L2–PE','L2–N','L3–PE','L3–N','L1–L2','L1–L3','L2–L3'],isolationRows.length?isolationRows:[['-','-','-','-','-','-','-','-','-','-','-','-']],{columnStyles:{0:{cellWidth:12},1:{cellWidth:15},2:{cellWidth:16},3:{cellWidth:16},4:{cellWidth:16},5:{cellWidth:16},6:{cellWidth:16},7:{cellWidth:16},8:{cellWidth:16},9:{cellWidth:16},10:{cellWidth:16},11:{cellWidth:16}},fontSize:5.1});
    if(defectRows.length){section('Stromkreisbezogene Mängel');table(['Nr.','UV','Stromkreis','Priorität','Feststellung / Maßnahme','Fotos'],defectRows,{columnStyles:{0:{cellWidth:14},1:{cellWidth:24},2:{cellWidth:32},3:{cellWidth:20},4:{cellWidth:84},5:{cellWidth:16,halign:'center'}},fontSize:6.5})}
    section('Mängel und Ergebnis');table(['Angabe','Dokumentation'],[['Allgemeine Mängel / Hinweise',f.maengel||'Keine Angaben'],['Ergebnis',f.ergebnis||'-'],['Prüfplakette',f.plakette==='ja'?'Ja – angebracht':f.plakette==='nein'?'Nein':'Nicht bewertet'],['Nächster Prüftermin',deDate(f.next)]],{columnStyles:{0:{cellWidth:44,fontStyle:'bold'}},fontSize:8});
    ensure(27);const signatureGap=6,signatureWidth=(pageW-margin*2-signatureGap*2)/3,masterX=margin+signatureWidth+signatureGap,customerX=masterX+signatureWidth+signatureGap,lineY=y+18;doc.setFont('helvetica','bold');doc.setFontSize(7.5);doc.text('Unterschrift Prüfer',margin,y);doc.text(`Meister: ${f.masterName||'—'}`,masterX,y,{maxWidth:signatureWidth});doc.text('Unterschrift Auftraggeber / Betreiber',customerX,y,{maxWidth:signatureWidth});doc.setDrawColor(90);doc.line(margin,lineY,margin+signatureWidth,lineY);doc.line(masterX,lineY,masterX+signatureWidth,lineY);doc.line(customerX,lineY,customerX+signatureWidth,lineY);if(d.sig&&d.sig.startsWith('data:image')){try{doc.addImage(d.sig,'PNG',margin+2,y+2,signatureWidth-4,14,undefined,'FAST')}catch(_){/* Unterschrift bleibt über der Linie frei */}}doc.setFont('helvetica','normal');doc.setFontSize(6);doc.setTextColor(90);doc.text('Datum / Name / Unterschrift',margin,lineY+4);doc.text('Datum / Name / Unterschrift',masterX,lineY+4);doc.text('Datum / Name / Unterschrift',customerX,lineY+4);doc.setTextColor(31,41,51);y+=26;
    const allPhotos=reportPhotos(d);if(allPhotos.length){section('Fotodokumentation');const photoGap=8,photoWidth=(pageW-margin*2-photoGap)/2,photoHeight=50;let x=margin,col=0;for(let i=0;i<allPhotos.length;i++){const photo=allPhotos[i];if(col===2){y+=photoHeight+9;x=margin;col=0}ensure(photoHeight+9);try{doc.addImage(photo.dataUrl,'JPEG',x,y,photoWidth,photoHeight,undefined,'FAST')}catch(_){try{doc.addImage(photo.dataUrl,'PNG',x,y,photoWidth,photoHeight,undefined,'FAST')}catch(__){/* Bild überspringen */}}doc.setFontSize(7);doc.text(photo.caption||photo.name||`Foto ${i+1}`,x,y+photoHeight+4,{maxWidth:photoWidth});x+=photoWidth+photoGap;col++}y+=photoHeight+9}
...
function pdfCircuitRows(number,u,r,c,hasUpstreamRcd,protectionRows,measurementRows,isolationRows){const hasRcd=hasUpstreamRcd||c.device==='fils',ce=circuitEvalText(c,hasRcd,r),kind=c.device==='fils'?'FI/LS':'LS',fi=c.device==='fils'?'integriert':hasUpstreamRcd?(r&&r.name||'FI'):'—',device=c.device==='fils'?`FI/LS ${(c.char||'')}${c.a||''} A · Typ ${c.rcdType||'A'} · ${c.rcdIdn||'30'} mA`:`LS ${(c.char||'')}${c.a||''} A`;protectionRows.push([number,u.name||'-',c.name||'-',c.cable||'-',`${c.cores||'-'} / ${deNumber(c.cross)}`,kind,fi,device,ce.text]);measurementRows.push([number,deNumber(c.rpe),deNumber(c.riso),deNumber(c.risoV),deNumber(c.zi),deNumber(c.zs),deNumber(c.ik),deNumber(c.rcdUl),deNumber(c.rcdms),deNumber(c.rcdma),pdfSafeText(c.note||ce.msg||'-')]);isolationRows.push([number,c.consumer==='ja'?'Ja':c.consumer==='nein'?'Nein':'-',...risoKeys.map(key=>deNumber(c[key]))])}
function protectionLabel(c, hasUpstreamRcd, r){
  ...
  if(hasUpstreamRcd){return {kind:'LS',fi:esc((r&&r.name)||'FI'),device:`LS ${esc((c.char||'')+(c.a||'')+'A')}`}}
  return {kind:'LS',fi:'—',device:`LS ${esc((c.char||'')+(c.a||'')+'A')}`};
}
...
function insulationRow(n,c){return `<tr><td>${n}</td><td>${esc(deNumber(c.risoV))}</td><td>${c.consumer==='ja'?'Ja':c.consumer==='nein'?'Nein':'-'}</td>${risoKeys.map(key=>`<td>${esc(deNumber(c[key]))}</td>`).join('')}</tr>`}
...
function captureInitialFieldValues(){
  fields.forEach(id=>{const e=document.getElementById(id);if(!e)return;initialFieldValues[id]=e.type==='checkbox'?e.checked:e.type==='radio'?false:e.value});
}
...
function updateStepProgress(){
  ...
  if(currentStep===3){const selectors=['#hakName','#hakPlace','#hakFuse','#hakSource','#hakCable','#hakCores','#hakCross','#voltageDrop','#earthResistance','#paResult'];total=selectors.length;completed=selectors.filter(s=>document.querySelector(s)?.value.trim()).length}
  if(currentStep===4){const circuits=[...document.querySelectorAll('.circuit')];total=Math.max(1,circuits.length);completed=circuits.filter(c=>{const hasRcd=!!c.closest('.rcd')||val(c,'.ck-device')==='fils';return val(c,'.ck-name')&&!isNaN(effectiveRisoElement(c))&&(hasRcd?(val(c,'.ck-rcdms')&&val(c,'.ck-rcdma')):(val(c,'.ck-zs')||val(c,'.ck-ik')))}).length;label=circuits.length?`${completed} von ${circuits.length} Stromkreisen vollständig`:'Noch kein Stromkreis'}
  if(currentStep===5){total=4;completed=(document.getElementById('ergebnis').value?1:0)+(document.getElementById('plakette').value?1:0)+(document.getElementById('next').value?1:0)+(document.getElementById('protocolStatus').value==='completed'?1:0)}
...
async function updateBackupReminder(protocols){const box=document.getElementById('backupReminder');if(!box||!database)return;const all=protocols||await dbAll('protocols'),completed=all.filter(item=>item.status==='completed');if(!completed.length){box.classList.add('hidden');return}const saved=await dbGet('settings','last-backup'),at=saved&&saved.at||'',newer=completed.filter(item=>(item.updatedAt||item.completedAt||'')>at).length,age=at?Math.floor((Date.now()-new Date(at).getTime())/86400000):Infinity;if(!at||newer||age>=14){box.classList.remove('hidden');box.innerHTML=`<b>Sicherung empfohlen</b><br>${!at?'Noch keine Sicherung erstellt.':newer?`${newer} abgeschlossene Prüfung${newer===1?' wurde':'en wurden'} seit der letzten Sicherung geändert.`:`Letzte Sicherung vor ${age} Tagen.`}`}else box.classList.add('hidden')}
...
async function saveProtocolForm(event){
  ...
  const data=makeBlankData({...(company&&company.data||{}),kunde:site.customer||'',objekt:site.name||'',adresse:site.address||'',datum:document.getElementById('newProtocolDate').value,startTime:new Date().toTimeString().slice(0,5),orderNo:site.orderNo||'',erst:type==='erst',wieder:type==='wieder',aenderung:type==='aenderung',erweiterung:type==='erweiterung',instandsetzung:type==='instandsetzung'});
  const typeLabels={erst:'Neuanlage',wieder:'Wiederholungsprüfung',aenderung:'Änderung',erweiterung:'Erweiterung',instandsetzung:'Instandsetzung'},subject=document.getElementById('newProtocolTitle').value.trim(),typeLabel=typeLabels[type]||'Prüfung';
...
async function openProtocol(protocolId){
  ...
  const protocolData=protocol.data||makeBlankData(),preset=await dbGet('settings','company');protocolData.fields=protocolData.fields||{};[...deviceFields,'masterName'].forEach(id=>{if(!protocolData.fields[id]&&preset&&preset.data)protocolData.fields[id]=preset.data[id]||''});
  loadData(protocolData);setSaveState('saved','Gespeichert');setEditorLocked(protocol.status==='completed');window.scrollTo({top:0,behavior:'smooth'});
}
...
async function exportBackup(){
  ...
  const blob=new Blob([JSON.stringify(backup,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`schaefchen-sicherung-${new Date().toISOString().slice(0,10)}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);await dbPut('settings',{key:'last-backup',at:nowIso()});await updateBackupReminder(protocols);showToast('Sicherungsdatei heruntergeladen');
}
