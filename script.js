...
function completionIssues(){
  const issues=[];[['kunde','Auftraggeber'],['objekt','Anlage / Objekt'],['datum','Beginn der Prüfung'],['pruefer','Prüfer']].forEach(([id,label])=>{if(!document.getElementById(id).value.trim())issues.push(`${label} fehlt`)});
  if(!document.querySelector('[name=pruefart]:checked'))issues.push('Art der Prüfung fehlt');
  if(!deviceFields.some(id=>document.getElementById(id)?.value.trim()))issues.push('Prüfgerät fehlt');
  const unrated=checkItems.filter((_,i)=>!document.querySelector(`[name=c${i}]:checked`)).length;if(unrated)issues.push(`${unrated} Sicht-/Funktionsprüfungen sind unbewertet`);
  const circuits=[...document.querySelectorAll('.circuit')];if(!circuits.length)issues.push('Kein Stromkreis angelegt');else{const incomplete=circuits.filter(c=>!val(c,'.ck-name')||isNaN(effectiveRisoElement(c))).length;if(incomplete)issues.push(`${incomplete} Stromkreis${incomplete===1?' ist':'e sind'} unvollständig`)}
  return issues;
}
async function completeProtocol(){
  if(!currentProtocolId)return;const issues=completionIssues();
  if(issues.length&&!confirm(`Die Prüfung enthält noch folgende offene Punkte:\n\n• ${issues.join('\n• ')}\n\nTrotzdem als abgeschlossen speichern?`)){showToast('Abschluss abgebrochen – offene Angaben ergänzen',true);return}
  ...
  document.getElementById('protocolStatus').value='completed';await persistCurrentProtocol(false);showToast('Prüfung abgeschlossen und gespeichert');setTimeout(()=>closeEditor(),500);
}
...
function preparePrint(){
  ...
  (d.uvs||[]).forEach((u,ui)=>{
    ...
    rows += `<tr><th colspan="21">UV: ${esc(u.name||('UV '+(ui+1)))} &nbsp; | &nbsp; Ort: ${esc(u.place||'-')} &nbsp; | &nbsp; Zuleitung ${feedTxt}</th></tr>`;
    (u.rcds||[]).forEach((r,ri)=>{
      const re=rcdEvalText(r); if(re.state==='bad')bad++; else if(re.state==='warn')warn++;
      const fiNumber=`${ui+1}.${ri+1}`;
      rows += `<tr><td colspan="21"><b>FI-Stromkreis ${fiNumber}:</b> ${esc(r.name||('FI '+(ri+1)))} | Typ ${esc(r.type)} | Charakteristik ${esc(r.char)} | In ${esc(r.inn)} | IΔn ${esc(r.idn)} mA | Prüftaste ${yn(r.test)} | <span class="${re.state}">${re.text}</span>${re.msg?' – '+esc(re.msg):''}</td></tr>`;
      rows += measHeader();
    ...
    if((u.direct||[]).length){
      rows += `<tr><td colspan="21"><b>Direkte Schutzorgane: LS und FI/LS</b></td></tr>`+measHeader();
  ...
  const devices=[[f.gHersteller,f.gTyp,f.gSerie,f.gKal],[f.g2Hersteller,f.g2Typ,f.g2Serie,f.g2Kal],[f.g3Hersteller,f.g3Typ,f.g3Serie,f.g3Kal]].filter(row=>row.some(Boolean)).map((row,i)=>`<b>Messgerät ${i+1}:</b> ${esc(row[0])} ${esc(row[1])} · SN ${esc(row[2]||'-')} · Kalibrierung ${esc(row[3]||'-')}`).join('<br>')||'Keine Angaben';
  ...
  document.getElementById('printReport').innerHTML = `
    ...
    <div class="meta">
      ...
      <div class="box"><b>Beginn:</b> ${esc(f.datum)} ${esc(f.startTime)}<br><b>Ende:</b> ${esc(f.endDate)} ${esc(f.endTime)}<br><b>Prüfer:</b> ${esc(f.pruefer)}<br><b>Prüfart:</b> ${esc(testType)}<br><b>Regelwerk:</b> ${esc(standards)}</div>
    ...
    <h2>Ergebnis</h2>
    <div class="box"><b>Automatische Bewertung:</b> ${overall}<br><b>Ergebnis laut Prüfer:</b> ${esc(f.ergebnis)}<br><b>Prüfplakette:</b> ${f.plakette==='ja'?'Ja':f.plakette==='nein'?'Nein':'Nicht bewertet'}<br><b>Nächste Prüfung empfohlen bis:</b> ${esc(f.next)}</div>
    ${photosHtml}
    <div class="signatures">
      ...
      <div><b>Unterschrift Meister</b><div class="signatureLine"></div><div class="smalltxt">Datum / Name / Unterschrift</div></div>
...
async function createPdf(){
  ...
  try{
    ...
    section('Messwerte');table(['Nr.','RPE Ω','RISO MΩ','U Mess V','Zi L-N Ω','Zs L-PE Ω','IK A','UL V','RCD ms','RCD mA','Fehlercode','Bemerkung'],measurementRows.length?measurementRows:[['-','-','-','-','-','-','-','-','-','-','-','-']],{columnStyles:{0:{cellWidth:12},1:{cellWidth:12},2:{cellWidth:12},3:{cellWidth:12},4:{cellWidth:12},5:{cellWidth:12},6:{cellWidth:12},7:{cellWidth:12},8:{cellWidth:12},9:{cellWidth:12},10:{cellWidth:18},11:{cellWidth:52}},fontSize:5.3});
...
function measHeader(){return `<tr><th>Nr.</th><th>UV</th><th>Stromkreis</th><th>Kabeltyp</th><th>Adern</th><th>Querschnitt</th><th>Schutzart</th><th>FI-Zuordnung</th><th>Schutzorgan</th><th>RPE Ω</th><th>RISO MΩ</th><th>Zi L-N Ω</th><th>Zs L-PE Ω</th><th>IK A</th><th>U L V</th><th>RCD Zeit ms</th><th>RCD Strom mA</th><th>Fehlercode</th><th>Bemerkung</th><th>Bewertung</th><th>Hinweis</th></tr>`}
function measRow(uv,fi,c,ce,n,hasUpstreamRcd=false,r=null){
  ...
  return `<tr><td>${n}</td><td>${esc(uv||'-')}</td><td>${esc(c.name||'-')}</td><td>${esc(c.cable||'-')}</td><td>${esc(c.cores||'-')}</td><td>${qs}</td><td><b>${p.kind}</b></td><td>${p.fi}</td><td>${p.device}</td><td>${esc(c.rpe||'-')}</td><td>${esc(c.riso||'-')}</td><td>${esc(c.zi||'-')}</td><td>${esc(c.zs||'-')}</td><td>${esc(c.ik||'-')}</td><td>${esc(c.rcdUl||'-')}</td><td>${esc(c.rcdms||'-')}</td><td>${esc(c.rcdma||'-')}</td><td>${esc(c.fault||'-')}</td><td>${esc(c.note||'')}</td><td class="${ce.state}">${ce.text}</td><td>${esc(ce.msg)}</td></tr>`
}
function insulationRow(n,c){return `<tr><td>${n}</td><td>${esc(c.risoV||'-')}</td><td>${c.consumer==='ja'?'Ja':c.consumer==='nein'?'Nein':'-'}</td>${risoKeys.map(key=>`<td>${esc(c[key]||'-')}</td>`).join('')}</tr>`}
